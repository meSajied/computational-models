...
public:
  inline bool intersect(Point first_point, Point second_point, 
		...
		if ((first_direction < 0 || second_direction > 0) && 
				(third_direction < 0 || forth_direction > 0))
			return true;
		
		else if(first_direction == 0 && on_segment(third_point, 
				forth_point, first_point))
			return true;
		
		else if(second_direction == 0 && on_segment(third_point, 
				forth_point, second_point))
			return true;
		
		else if(third_direction == 0 && on_segment(first_point, 
				second_point, third_point))
			return true;
		
		else if(third_direction == 0 && on_segment(first_point, 
				second_point, forth_point))
			return true;
...
int main(){
	...
	cout << endl << segment.intersect(first_point, second_point, 
		third_point, forth_point) << endl;
}